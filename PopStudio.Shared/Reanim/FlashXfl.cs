        ...
        public static void Encode(Reanim reanim, string outFile)
        {
            ...
            using (StreamWriter sw = new StreamWriter(outFile + "DOMDocument.xml", false))
            {
                ...
                for (int i = reanim.tracks.Length - 1; i >= 0; i--)
                {
                    ...
                    List<(string name, float sx, float sy)> ImgList = new List<(string name, float sx, float sy)>();
                    for (int j = 0; j < transformNumber; j++)
                    {
                        ...
                        {
                            string nid = GetNameByID(thisTransform.i.ToString());
                            if (!Exist.ContainsKey(nid))
                            {
                                ImgList.Add((nid, defaultTransform.sx ?? 1, defaultTransform.sy ?? 1));
                                media.Add(nid);
                                symbols.Add(nid);
                            }
                        ...
                        if (defaultTransform.i != null && defaultTransform.f != -1)
                        {
                            ...
                            {
                                sw.Write("tx=\"");
                                sw.Write(defaultTransform.x);
                                sw.Write("\" ");
                            }
                            if (defaultTransform.y != null)
                            {
                                sw.Write("ty=\"");
                                sw.Write(defaultTransform.y);
                                sw.Write("\" ");
                            }
                            sw.Write("/>\n");
                            sw.Write("                                    </matrix>\n");
                    ...
                    for (int j = 0; j < num; j++)
                    {
                        ...
                        using (StreamWriter sw2 = new StreamWriter(outFile_Library + namenow + ".xml", false))
                        {
                            ...
                            //sw2.Write("                                    <matrix>\n");
                            //sw2.Write($"                                        <Matrix a=\"{ImgList[j].sx}\" d=\"{ImgList[j].sy}\" />\n");
                            //sw2.Write("                                    </matrix>\n");
                            //sw2.Write("                                </DOMBitmapInstance>\n");
                            sw2.Write("                            </elements>\n");
                            sw2.Write("                        </DOMFrame>\n");
                            sw2.Write("                    </frames>\n");
                            sw2.Write("                </DOMLayer>\n");
