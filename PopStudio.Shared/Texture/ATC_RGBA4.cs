...
{
    internal static unsafe class ATC_RGBA4
    {
        public static YFBitmap Read(BinaryStream bs, int width, int height)
        {
            ...
            for (int y = 0; y < height; y += 4)
            {
                ...
                {
                    ulong aword = bs.ReadUInt64(Endian.Small);
                    for (int j = 0; j < 4; j++)
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            ulong temp = aword & 0xF;
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        color[i] = color_buffer[colorFlags & 0b11].WithAlpha(alpha[i]);
                        colorFlags >>= 2;
                    }
                    //赋值
                    for (int i = 0; i < 4; i++)
        ...
        public static int Write(BinaryStream bs, YFBitmap image)
        {
            ...
            for (int i = 0; i < height; i += 4)
            {
                for (int w = 0; w < width; w += 4)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        for (int k = 0; k < 4; k++)
                        {
                            if ((i + j) < height && (w + k) < width)
                            {
                                ...
                                temp |= ((ulong)(color[n].Alpha >> 4)) << (n << 2);
                            }
                            else
                            {
                    ...
                    bs.WriteUInt64(temp, Endian.Small);
                    //Color code
