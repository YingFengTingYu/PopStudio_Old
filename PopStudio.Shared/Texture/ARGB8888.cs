...
namespace PopStudio.Texture
{
    internal static class ARGB8888
    {
        public static SKBitmap Read(BinaryStream bs, int width, int height)
        {
            ...
            {
                temp = bs.ReadInt32();
                pixels[i] = new SKColor((byte)((temp & 0xFF0000) >> 16), (byte)((temp & 0xFF00) >> 8), (byte)(temp & 0xFF), (byte)(temp >> 24));
            }
        ...
        public static int Write(BinaryStream bs, SKBitmap image)
        {
            ...
            for (int i = 0; i < S; i++)
            {
                bs.WriteInt32((pixels[i].Alpha << 24) | (pixels[i].Red << 16) | (pixels[i].Green << 8) | pixels[i].Blue);
            }
