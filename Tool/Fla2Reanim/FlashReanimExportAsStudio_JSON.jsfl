...
function MakeURI()
{
	...
	var filename = gDoc.path.substring(0, lastIndex) + ".reanim.json";
...
function WriteReanimFrame(sampleFrame, frameArray, prev, theTrack, elementIndex)
{
	...
	if (elementArray.length != 1)
	{
		...
		if (prev.prevF != -1)
		{
			theTrack.transforms.push({"f":-1})
			//FLfile.write(URI, "<t><f>-1</f></t>\n", "append");
			prev.prevF = -1;
		}
		else
		{
			theTrack.transforms.push({})
	...
	*/
	...
	{
		theTransform.x = Number(stringX)
	}
	
	if (prev.prevY != stringY)
	{
		theTransform.y = Number(stringY)
	}
							
	if (prev.prevKX != stringDegreesKX)
	{
		theTransform.kx = Number(stringDegreesKX)
	}
	
	if (prev.prevKY != stringDegreesKY)
	{
		theTransform.ky = Number(stringDegreesKY)
	}
				
	if (prev.prevSX != stringSX)
	{
		theTransform.sx = Number(stringSX)
	}
	
	if (prev.prevSY != stringSY)
	{
		theTransform.sy = Number(stringSY)
	}
	
	if (prev.prevF != stringF)
	{
		theTransform.f = Number(stringF)
	}
	
	if (prev.prevA != stringA)
	{
		theTransform.a = Number(stringA)
	}

	if (prev.prevImageName != imageName)
	{
		theTransform.i = imageName
	}
	 
	if (prev.prevFontName != fontName)
	{
		theTransform.font = fontName
	}
	...
	if (prev.prevText != stringText)
	{
		...
		else
		{
			...
			theTransform.text = stringText
		}			
	}
	 
	//FLfile.write(URI, "</t>\n", "append");
	theTrack.transforms.push(theTransform)
...
function WriteReanimLayer(layerIndex, theTrack)
{	
	...
	for (elementIndex = 0; elementIndex < maxElements; elementIndex++)
	{
		...
		{	
			numElements = WriteReanimFrame(sampleFrame, frameArray, prev, theTrack, elementIndex);
...
function WriteReanimFile()
{
	...
	// prepare reanim object
	ReanimObject = {}

	ReanimObject.fps = gDoc.frameRate	
	...
	{
		var theTrack = {}
		WriteReanimLayer(layerIndex, theTrack);
		ReanimObject.tracks.push(theTrack)
	}
	FLfile.write(URI, JSON.stringify(ReanimObject), "append")
	fl.trace("done writing file");
	//fl.trace(ReanimObject.tracks.length)
}
